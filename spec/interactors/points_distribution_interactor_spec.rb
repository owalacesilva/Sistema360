...
RSpec.describe PointsDistributionInteractor do
  let(:user) { create(:user_spilled, qualify: "rubi") }
  ...
  context "with valid parameters" do
    ...
    it "is expected point dist to be done" do
      expect(point_dist.reload.done).to eq(true)
    end
  end
  ...
  context "without valid reference" do
    ...
    it "does not change point_dist object" do
      expect(point_dist.reload.done).to be_falsey
    end
  ...
  context "when reference points is from activation" do
    ...
    it "is expected that user point is 30.0 points" do
      expect(user_point(user).reload.amount).to eq(20.0)
    end
  end
end
