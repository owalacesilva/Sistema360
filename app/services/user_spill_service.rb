class UserSpillService
  prepend SimpleCommand

  attr_reader :user_spill, :user, :user_sponsor

  def initialize(user_spill)
    @user_spill = user_spill
    @user = user_spill.user
    @user_sponsor = user_spill.user_sponsor
  end
  ...
  def parent_node_right_latest
    parent_node_latest = UserNetwork.right_latest
    parent_node_latest.rgt + 1
  end

  def update_all_nodes_before_insert(target)
    UserNetwork.nodes_from_target(target).each do |node|
  ...
  def create_user_network_node(user_parent_node, user, target)
    ...
    user_node = UserNetwork.new(user: user, sponsor_node: user_parent_node, parent_node: user_parent_node,
